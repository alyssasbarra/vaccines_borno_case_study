...
tabulate_lbd <- function(nid, collapse_method="kish") {
  ...
  if (dataset[[1]] != FALSE){ 
    ...
    if(max(data$year_end) >= 2000){  # Include surveys from 2000 onwards
      ...
      # Identify failed geomatching (interactive)
      # fix <- copy(data)
      # fix[, num_clusters := uniqueN(geo_id), by = c("svy_id", "country", "year_id", "survey_name")]
      # fix <- fix[is.na(shapefile) & (is.na(latitude) | is.na(longitude)), ]
      # fix_collapse <- fix[, .("num_unmatched_clusters" = uniqueN(geo_id)), 
      #                     by = c("svy_id", "country", "year_id", "survey_name", "num_clusters")]
      # fix_total <- fix[, .("geo_id" = unique(geo_id)), by=.(svy_id, country, year_id, survey_name)]
