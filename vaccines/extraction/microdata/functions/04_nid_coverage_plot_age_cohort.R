...
# Author:  USERNAME
# Date:    DATE
# Purpose: For LBD - read tabulated, pre-resampled data for each nid. For a list of covariates, plot coverage on country map.
# Run:     source("FILEPATH")
# Inputs:  nid  ->  NID from dataset in 'FILEPATH' to tabulate; vaccine covs to plot
#***********************************************************************************************************************
library(data.table)
    ...
    if (nrow(df_pointpoly) > 0) {
      ...
      for (vax in covs) {
        ...
        if (length(dose_vars) > 0) {
          ...
          # Convert from dose-specific to dose-aggregate
          ...
          # shapefile + survey_name + year_id + age_bin + psu + weight ~ me_name, sum,value.var = c("value", "N")))
          ...
          if (cov_var %in% names(df)) {
            ...
            outlier    <- outlier[lbd == 1 & (me_name == "" | me_name %in% dose_vars),] #keep only lbd outliers specific to mcv or all vaccines
            ...
            year_list <- 1980:2020 #c(2000:2017)
            ...
            # Produce one map for each age bin in data
            ...
            for(age_cohort in age_cohorts){
              ...
              gg_svy <- gg_svy +
                ...
                theme_void() +
                coord_equal() +
                #labs(title = paste0(toupper(vax_prefix), vax_dose)) +
              ...
              # Pull a color legend from gg_svy
              ...
              #gg_est <- gg_est + theme(legend.position = "none")
