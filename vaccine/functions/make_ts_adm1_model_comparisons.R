make_ts_adm1_model_comparisons <- function(indicator_group = indicator_group,
                                           indicator       = indicator,
                                           run_date        = run_date,
  ...
  #                         indicator       = "",
  #                         indicator_group = "",
  #                         config_file     = paste0('FILEPATH),
  #                         covs_file       = paste0('FILEPATH'),
  #                         run_tests       = TRUE)
...
for(i in rev(1:length(models))) {
  ...
  dir.create(out_dir)
  out_filename_format = "subnational_model_comparison_%s.pdf"
  ...
  if (plot_agebin_faceted==TRUE){
    ...
    for (i in ad0_shape_simple$ADM0_CODE) {
      ...
      for (j in ad1_shape_simple[ad1_shape_simple$ADM0_CODE==i,]$ADM1_CODE) {
        ...
        gg_ad0_ts <-
          ggplot() +
          geom_line(data = model_data, aes(x = year, y = mean, color=as.factor(run), lty=model)) +
  ...
  if (plot_year_faceted==TRUE) {
    ...
    for (i in ad0_shape_simple$ADM0_CODE) {
      ...
      for (j in ad1_shape_simple$ADM1_CODE) {
        ...
        gg_ad0_ts<-
          ggplot() +
          geom_line(data = model_data, aes(x = as.numeric(agebin), y = mean, color=as.factor(run), lty=model)) +
          theme_bw(base_size = 16)+
