make_ts_adm1_plots_age_faceted <- function(indicator_group = indicator_group,
                                           indicator       = indicator,
                                           run_date        = run_date,
  ...
  input_data <- fread(paste0(in_dir, 'processed_input_data.csv'))
  ...
  if (plot_agebin_faceted==TRUE){
    pdf_filename <- paste0(out_dir, sprintf(out_filename_format, regions), "_by_admin1_x_as_year_2")
    ...
    for (i in ad0_shape_simple$ADM0_CODE) {
      ...
      for (j in ad1_shape_simple[ad1_shape_simple$ADM0_CODE==i,]$ADM1_CODE) {
        print(j)
        model_data=ad1_df[ADM0_CODE==i & ADM1_CODE==j]
        input_data=ad1_data[ADM0_CODE==i & ADM1_CODE==j]
        ...
        gg_ad0_ts <-
          ggplot() +
          geom_ribbon(data = model_data, aes(x = year, ymin = lower, ymax = upper, fill=as.factor(run)), alpha = 0.3) +
         # scale_linetype_identity() +
          geom_line(data = model_data, aes(x = year, y = mean, color=as.factor(run))) +
  ...
  if (plot_year_faceted==TRUE) {
    pdf_filename <- paste0(out_dir, sprintf(out_filename_format, regions), "_by_admin1_x_as_age")
    ...
    for (i in ad0_shape_simple$ADM0_CODE) {
      print(i)
      for (j in ad1_shape_simple$ADM1_CODE) {
        print(j)
        model_data=ad1_df[ADM0_CODE==i & ADM1_CODE==j]
        input_data=ad1_data[ADM0_CODE==i & ADM1_CODE==j]
        ...
        gg_ad0_ts<-
          ggplot() +
          geom_ribbon(data = model_data, aes(x = as.numeric(agebin), ymin = lower, ymax = upper, fill=as.factor(run)), alpha = 0.3) +
         # scale_linetype_identity() +
          geom_line(data = model_data, aes(x = as.numeric(agebin), y = mean, color=as.factor(run))) +
          theme_bw(base_size = 16)+
