...
get_is_oos_draws_agebins <- function(ind_gp,
  ...
  # load raw data
  if (!get.oos) {
    #  try(df <- fread(paste0(mod.dir, "input_data.csv"))) ## raw input data
    ...
    if(create_holdout_files==F & !is.null(holdout_run_date)){
      stratum_dir<-sprintf("FILEPATH")
      df <- rbindlist(lapply(Regions, function(reg){
        df<-readRDS(paste0(stratum_dir, "stratum_", reg, ".rds"))
        df<-setDT(df[[1]])
      })) 
    }else{
      df <- rbindlist(lapply(Regions, function(reg){
  ...
  df_all <- rbindlist(lapply(all.regions, function(rr) {
    ...
    for(a in agebins){
        for (this_fold in sort(unique(df.r$fold))) {
          ...
          message(sprintf('..load cell preds for age %i and and holdout %i', a, this_fold))
          filename_fold <- paste0(mod.dir, sprintf("%s_cell_draws_eb_bin%i_%s_%i.RData", ind, a, rr, this_fold))
          if(file.exists(filename_fold)){
            load(filename_fold)
            df.r[fold == this_fold & age_bin==a, paste0("draw", 1:ncol(cell_pred)) := as.data.table(cell_pred[cell_pred_id, ])]
            remove(cell_pred)
          } else {
