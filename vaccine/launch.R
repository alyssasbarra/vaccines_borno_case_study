...
##   qsub -e FILEPATH -o FILEPATH-cwd -pe multi_slot 16 -P proj_geospatial -N dpt3_cov FILEPATH FILEPATH
## Geo cluster as qsub:
##   qsub -e FILEPATH -o FILEPATH -N dpt1_cov_gn -P proj_geospatial -l gn=TRUE FILEPATH FILEPATH
###############################################################################
###############################################################################
...
cores <- 4
    ...
    qsub <- make_qsub_share(age               = loopvars[i,2],
                            ...
                            geo_nodes         = FALSE,
                            use_c2_nodes      = use_c2_nodes,
                            queue             = 'long.q',
                            proj              = 'proj_geospatial',
                          #  code='parallel_model3',
                            run_time       = '7:00:00:00')
...
##############################################################################
if (use_gp == T & use_stacking_covs == T & fit_with_tmb==F) {
