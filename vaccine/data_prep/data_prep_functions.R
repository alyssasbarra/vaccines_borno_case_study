...
# Author: USERNAME
# Date: DATE
# Project: MBG / Vaccine / All
# Purpose: Functions for use in prepping coverage data for MBG
# Details: 
# source("FILEPATH")
#**************************************************************************

# GEOPOSITIONING FUNCTIONS ------------------------------------------------
...
geoposition_data <- function(survey_type, df, country_specific = F, error_dir) {
  ...
  #
  # Note: currently *drops* non-positioned data points!
  
  # 1. Create geographies database -----------------

  if (country_specific == T) {
   geo_file <- paste0("FILEPATH")  
  ...
  #   For now, not using the pointpoly-extracted lat/longs 
